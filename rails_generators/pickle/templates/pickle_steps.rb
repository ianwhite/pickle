...
# find a model
When.new(/^#{capture_model} should exist(?: with #{capture_fields})?$/) do |name, fields|
  find_model!(name, fields)
end

# not find a model
When.new(/^#{capture_model} should not exist(?: with #{capture_fields})?$/) do |name, fields|
  expect(find_model(name, fields)).to be_nil
end

# find models with a table
When.new(/^the following #{capture_plural_factory} should exist:?$/) do |plural_factory, table|
  expect(find_models_from_table(plural_factory, table)).not_to be_any(&:nil?)
end

# not find models with a table
When.new(/^the following #{capture_plural_factory} should not exists?:?$/) do |plural_factory, table|
  find_models_from_table(plural_factory, table).should be_all(&:nil?)
end

# find exactly n models
When.new(/^(\d+) #{capture_plural_factory} should exist(?: with #{capture_fields})?$/) do |count, plural_factory, fields|
  expect(find_models(plural_factory.singularize, fields).size).to eq(count.to_i)
end

# assert equality of models
When.new(/^#{capture_model} should be #{capture_model}$/) do |a, b|
  expect(model!(a)).to eq(model!(b))
end

# assert model is in another model's has_many assoc
When.new(/^#{capture_model} should be (?:in|one of|amongst) #{capture_model}(?:'s)? (\w+)$/) do |target, owner, association|
  expect(model!(owner).send(association)).to include(model!(target))
end

# assert model is not in another model's has_many assoc
When.new(/^#{capture_model} should not be (?:in|one of|amongst) #{capture_model}(?:'s)? (\w+)$/) do |target, owner, association|
  expect(model!(owner).send(association)).not_to include(model!(target))
end

# assert model is another model's has_one/belongs_to assoc
When.new(/^#{capture_model} should be #{capture_model}(?:'s)? (\w+)$/) do |target, owner, association|
  expect(model!(owner).send(association)).to eq(model!(target))
end

# assert model is not another model's has_one/belongs_to assoc
When.new(/^#{capture_model} should not be #{capture_model}(?:'s)? (\w+)$/) do |target, owner, association|
  expect(model!(owner).send(association)).not_to eq(model!(target))
end

# assert model.predicate?
When.new(/^#{capture_model} should (?:be|have) (?:an? )?#{capture_predicate}$/) do |name, predicate|
...
When.new(/^#{capture_model} should not (?:be|have) (?:an? )?#{capture_predicate}$/) do |name, predicate|
  if model!(name).respond_to?("has_#{predicate.gsub(' ', '_')}")
...
When.new(/^#{capture_model}'s (\w+) (should(?: not)?) be #{capture_value}$/) do |name, attribute, expectation, expected|
...
When.new /^#{capture_model} should have (\d+) (\w+)$/ do |name, size, association|
