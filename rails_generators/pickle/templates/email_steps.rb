...
When.new(/^(?:I|they) follow "([^"]*?)" in #{capture_email}$/) do |link, email_ref|
  visit_in_email(email(email_ref), link)
end

When.new(/^(?:I|they) click the first link in #{capture_email}$/) do |email_ref|
  click_first_link_in_email(email(email_ref))
end

When.new(/^(\d)+ emails? should be delivered to (.*)$/) do |count, to|
...
When.new(/^(\d)+ emails? should be delivered with #{capture_fields}$/) do |count, fields|
